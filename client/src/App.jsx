...
// Build all other URLs from the base URL
const API_URL = `${API_BASE_URL}/history`;
const API_NEXT_SPIN_TIME_URL = `${API_BASE_URL}/next-spin-time`;
const API_NEXT_SPIN_NUMBER_URL = `${API_BASE_URL}/next-spin-number`;
...
function App() {
  ...
  const [forcedNumber, setForcedNumber] = useState(() => {
    const savedForcedNumber = localStorage.getItem("forcedAdminNumber");
    return savedForcedNumber !== null ? JSON.parse(savedForcedNumber) : null;
  });
  ...
  // MODIFICATION: Save state to localStorage on change
  useEffect(() => {
    if (forcedNumber === null) {
      localStorage.removeItem("forcedAdminNumber");
    } else {
      localStorage.setItem("forcedAdminNumber", JSON.stringify(forcedNumber));
    }
  }, [forcedNumber]);
    ...
    fetch(API_NEXT_SPIN_NUMBER_URL)
      ...
      .then((data) => {
        ...
        setTimeout(() => {
          ...
          // Play the win sound as the result is revealed
